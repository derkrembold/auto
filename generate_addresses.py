...
def render_python(data):
    ...
    lines.append("")
    lines.append("    # pid value -> its own message name, for human-readable logging")
    lines.append("    # (e.g. watchdog/linbus.py's --debug bus-call tracing).")
    pid_name_pairs = ", ".join(f"{m['name']}: {m['name']!r}" for m in messages)
    lines.append(f"    pid_names = {{{pid_name_pairs}}}")
    lines.append("")
