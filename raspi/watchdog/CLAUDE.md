...
## Multi-Instance Addressing (2026-09-11)

**Every motor- and current-sensor-scoped command now takes a required
instance number as its first argument, no default** — `speed 0 500`,
`pulse 1 1000`, `status 0`, `current 1`, etc. Built as the foundation
for the second motor (a spare board with dead MOSFETs but a working
MCU/Hall sensors, jumpered as a safe instance-1 test bed to confirm
addressing works before any real second power stage exists). Decided by
discussion, step by step, before writing anything:

- **Instance is a hard requirement, never a silent default** — matches
  this project's existing "explicit over implicit" bias (e.g. `errors`
  staying a separate manual command instead of auto-triggering after a
  failed `current` read). A missing/out-of-range instance is a
  `validate()` usage error, not a silent fall-through to instance 0.
- **Range mirrors the addressing scheme's own reserved capacity**
  (`addresses.json`'s `instances` list, root `CLAUDE.md`'s LIN Protocol
  section) — motor 0-3, current sensor 0-1. `MOTOR_INSTANCE_MIN/MAX`,
  `CURRENT_INSTANCE_MIN/MAX` in `watchdog.py`.
- **`linbus.py`**: every command function (`set_speed`, `get_hal`,
  `get_current`, etc.) now takes `instance` as a required parameter,
  resolved to the wire id via `_motor_wire_id()`/`_current_wire_id()`
  (`constants.motor_instances[n]`/`constants.current_instances[n]`,
  already generated by `generate_addresses.py` — no changes needed
  there or to `addresses.json`, that capacity was already reserved).
  The old single hardcoded `TARGET_MOTOR_INSTANCE`/`MOTOR_INSTANCE_ID`/
  `CURRENT_INSTANCE_ID` module constants are gone.
- **`selftest` deliberately NOT instance-parameterized yet** (staged,
  agreed with the user): its internal `linbus` calls now pass
  `SELFTEST_MOTOR_INSTANCE`/`SELFTEST_CURRENT_INSTANCE` (both 0)
  explicitly, so it keeps working exactly as before — no CLI change.
...
- **Every script that talks to the watchdog directly now has a
  ...
  `--current-instance`), `characterize_hall_positions.py` (interactive
  prompt, default 0). `run_experiment.py`/`run_grid.py`/`run_grid_row.py`
  needed **no changes at all** — they only ever invoke
  `capture_step_response.py` over SSH by command line, never touch the
  watchdog socket directly, so its new `--motor 0` default already
  covers them.
- **176→198 tests, all green** (`raspi/tests/test_watchdog.py` and
  `test_capture_step_response.py` mechanically updated to the new
  command shape, plus new multi-instance-specific cases — different
  instances resolve to different wire pids, cross-instance isolation of
